...
public class ConfigurableControllerEquipAnimationData {
    ...
    float equipAnimationSpeed = 1 / 5f;
    
    float nextAnimationDelta = 0;
    
    Pair<Vec3, Vec3> modelEquipRotation = Pair.of(new Vec3(-20, 180, -2.5), new Vec3(-35, 145, -10));
    ...
    public Vec3 getModelPosition(float pt) {
        return interpolateVectorPair(modelEquipPosition,  (equipAnimation + nextAnimationDelta * pt) * 0.8f);
    }
    
    public void tick() {
        if (Minecraft.getInstance().isPaused() || activeStack == null) return;
        equipAnimation += nextAnimationDelta;
        float equipAnimationTarget = CreateConfigurableClient.CONTROLLER_HANDLER.isUsing() ? 1 : 0;
